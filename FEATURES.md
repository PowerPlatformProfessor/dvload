...
- **Several mappings open at once** on the Import tab: a chip per mapping above the Source section, *+ New mapping* to start another (added files stay shared), double-click a chip to rename it in place, and clicking a chip switches the whole editor — source, target, columns and options — to that mapping. The set survives closing the pane. *Save all → Run plan* writes every mapping to its `.dvmap.json` and turns the set into run-plan steps (updating a step in place when its mapping is re-saved, so stages and dependencies tuned on the other tab survive), then lands on the Run plan tab
- Plan editing is file-aware and visual: *Add current mapping as step* writes the `.dvmap.json` and points the step at it (workbook prefilled from the added source), cards are grouped under **Stage N** headings in execution order, a step's stage can be typed or changed by dragging its grip onto another stage's card, and *Run all steps* offers to save the plan before running it
- **Run all steps** executes the plan in the pane itself (Excel, `dvload gui`, and the Power Platform ToolBox): same stage/dependency ordering as `dvload run-all` (shared `buildExecutionBatches` in core), steps run sequentially with one confirm up front, per-step `dryRun`/`maxErrors`/`concurrency` overrides honored, `stopOnError` respected. Workbooks resolve by file name against the added files; `.dvmap.json` files are picked when the run starts. `refresh`, `notifyUrl` and `user` overrides are CLI-only and reported as ignored
